...
{: .warning }
> **This file is an archive and is not accurate.** It was retired on 2026-09-20. Nothing in it may be
> cited as a reason to do or not do anything, and it must not be used to answer a question about how
> the app works today. Several entries describe behaviour the build has not had for weeks, and at
> least one described a privacy property the code did not hold. Read the code, the tests, and
> [architecture.md](../architecture.md); the decisions that bind today are in
> [decisions.md](../decisions.md), which starts from scratch.
>
> It is kept only so nobody re-derives an argument that was already had.
