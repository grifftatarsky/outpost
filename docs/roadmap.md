...
Eight epics, 109 tickets, counted 2026-09-22.
...
| A packet carries only what its reader may read | Complete (tested) | `AppSession.mayReceive` decides, per entry, who may have it — a room's members, an Outpost's readers, nobody past a history floor, a removed member only what was sealed before the key turned — and the round writes one packet per audience. Repair answers only from the conversation asked about. A reader shown only part of a log is told which positions are withheld, and stops asking (`RepairScopeTests`). A round built this way crossed two Apple Accounts on 2026-09-21; keeping a second room's entries from a third member needs a third account to see on the rig. |
| The envelope names only its own conversation | Complete (tested) | An entry's clock names only its own conversation. A packet carries keys, certificates and revocations only for the people it names, and a wish about an Outpost goes only to its owner. Repair heads stay inside the conversation asked about (`EnvelopeLeakTests`, each case red with its rule removed). |
| A log per conversation | Complete (tested) | `FeedKey` is author, device and conversation, and `ConversationID` is an enum — room, solo, Outpost — that every entry carries. Positions count only within a conversation, so no number says anything about a writer's other conversations. A device's own head in each conversation is saved and never goes backward (`DeletingARoomTests.comingBackDoesNotReuseANumber`, red five times in five without it). A fold of three hundred entries across four rooms takes about 390 ms. **Proved on two Apple Accounts 2026-09-21**: onboarding, a room, an invitation with matching phrases, the key grant, and a message each way, the sender's mark moving to *Collected*. Coming back to a deleted room is held by the suite, not yet seen on the rig. A reinstall carries on as the same device — [the epic](epics/identity-and-devices.md#a-reinstall-is-the-same-device). |
| Members vouch for each other's logs | Complete (tested) | Every packet with entries attests the position and hash of each log's newest entry in those rooms, only to recipients who may already have it. A mismatch is recorded as a contradiction — not called a fork — and this device asks the attester for their copy; the author's signed entry is the proof. Quiet when the author recently asked to be restored, shown on the Integrity screen otherwise (`AttestationTests`, every case mutation-checked). **Crossed two Apple Accounts 2026-09-21**: each device heard about both logs in the room and recorded no contradiction. A contradiction itself has only been seen in the suite. |
...
| Unread, and the number on the icon | Complete (tested) | Dot and badge on beta 2026-09-02, cleared by reading. The extension's half rides on the push. **2026-09-17: the number ignored the member's own switches** — `BadgeCount` counted unread rooms unconditionally, never read `BadgeChoices` and never counted Outposts, so two controls on the Notifications screen did nothing and all four sentences of `BadgeMeaningLine` were false. Fixed, and it is one `AppSession.badgeNumber` the app and the extension share rather than a sum written twice ([Decisions](archive/decisions-2026.md#the-number-on-the-app-icon-is-one-property-and-it-obeys-the-switches)). Measured on Quad: 1 with an unread room, none once the switch was off with the message still unread, and back again. |
...
| Focus filters | Complete (hardware proof owed) | A filter per Focus: which rooms may notify, whether banners carry the words (`FocusFilterTests`). 2026-09-17, found while making the extension's decisions testable: *Show what was said* was applied to a message and not to a post, so a Focus set to hide it still put a post's body on the lock screen. Fixed — the preview switch reaches a post, the room list deliberately does not ([Decisions](archive/decisions-2026.md#a-focus-filters-preview-switch-reaches-a-post-and-its-room-list-does-not)). A simulator has no Focus — [Still to prove](#still-to-prove). |
...
<details markdown="1">
<summary><b>Your identity across your devices</b> · 13 tickets — 9 tested · 2 hardware proof owed · 1 canceled · 1 proved above the mailbox</summary>
...
| A reinstall is the same device | Complete (tested) | Built 2026-09-22 ([epic](epics/identity-and-devices.md#a-reinstall-is-the-same-device), [decision](decisions.md#a-reinstall-is-the-same-device-if-it-is-still-correctly-identified)). The device key never leaves the device and answers to one member; the log and state stay out of backups; a device with its keys and nothing else writes nothing until it has read its own records back from iCloud, then carries on from where it was. A small summary record per device holds its positions, keys, contacts and what only it keeps; a round sends its own writing only once that summary is saved. `ReinstallTests` and five more suites, every guard mutation-checked. **Proved on two Apple Accounts 2026-09-22**: deleted and reinstalled on alpha, it read its place back and wrote at the next position; Quad received it with no fork, and Quad's reply came back. |
...
| Light mode, from rules rather than drawings | Complete (tested) | Every swatch carries a real light value and the contrast audit covers both appearances. The sweep of surfaces the system draws was done 2026-09-15. 2026-09-17: every color a view picks for itself was measured in both appearances, with Increase Contrast on and off, for every accent — the destructive red, Increase Contrast fills, unlit marks, four filled buttons, accent words on cards, the anonymous face and white words on glass over photos were all fixed, seven audit tests added, and what stays is in [Decisions](archive/decisions-2026.md#a-color-is-measured-on-every-ground-it-is-drawn-on-in-both-appearances). |
...
`WideLayoutTests` — [Decisions](archive/decisions-2026.md#the-mac-and-an-ipad-at-full-width-share-one-layout-areas-a-list-and-what-is-open).
...
| The app icon on macOS | Complete (QA required) | Not yet seen in the Dock. An Icon Composer document with a dark appearance, scoped to the macOS SDK; checked in the compiled `.icns` at every size. The full mark does not read at 16 points. [Decisions](archive/decisions-2026.md#the-mac-icon-is-an-icon-composer-document-and-ios-keeps-its-own). |
| The desktop wall | Complete (QA required) | Not yet seen in a window. An inspector beside your own Outpost holding *Who sees your Outpost*, open by default, hidden with ⌥⌘I. [Decisions](archive/decisions-2026.md#the-desktop-wall-is-an-inspector-open-by-default-and-hideable). |
| A Settings window | Complete (QA required) | Panes drawn offscreen, the real window not yet opened. ⌘, opens a Settings window with a toolbar of panes — Appearance, Behavior, Notifications, Privacy & Safety, Outposts, Devices, Data — holding the same pages the iPhone pushes from You. You on the Mac keeps your name, People, Supporter, your Outpost, help, and a *Settings* row. [Decisions](archive/decisions-2026.md#the-mac-has-a-settings-window-built-from-the-same-pages). |
| What's waiting, in the toolbar | Complete (QA required) | Rendered offscreen, not yet seen live. The filled mailbox; a popover of unread rooms and new Outposts; a count that is the Dock's number and shows only when badges are on. Rendered offscreen at 3, 12 and 99+, unclipped. [Decisions](archive/decisions-2026.md#the-mac-toolbars-mailbox-carries-a-hand-drawn-count-and-only-when-badges-are-on). |
| The wide layout, Mac and iPad | Complete (tested) | Areas, the iPhone's own lists, and what is open, in three columns. The Mac's sidebar had no unread marks, pins, tags, search, waiting invitations or way to start a room; it has all of them now because the list is the iPhone's. [Decisions](archive/decisions-2026.md#the-mac-and-an-ipad-at-full-width-share-one-layout-areas-a-list-and-what-is-open). |
| Paste a photo | Complete (tested) | ⌘V with a copied photo or media file attaches it in the composer and a new post; words paste as before. [Decisions](archive/decisions-2026.md#v-with-a-photo-on-the-clipboard-attaches-it-and-words-still-paste-as-words). Not on an iPad. |
| Drag and drop | Complete (tested) | A photo or clip dragged onto a conversation or a new post is staged as the picker would stage it, and a path a text view inserts is turned back into the file. [Decisions](archive/decisions-2026.md#a-photo-or-clip-dragged-onto-a-conversation-or-a-new-post-is-attached-never-pasted-as-its-path). Not dragged by a real pointer. |
| The menu bar | Complete (QA required) | Structure dumped from a windowless probe; the menus have not been used against a window. File, View, Go and Help carry the app's commands with standard shortcuts, and a compose button in the sidebar starts a room, a solo or joins with an invite — the Mac had no way to do any of the three. [Decisions](archive/decisions-2026.md#the-macs-menu-bar-carries-the-apps-commands). |
...
- **Return and Shift-Return — done 2026-09-18.** Return sends; Shift-Return and Option-Return write a
  line, measured on the real field — [Decisions](archive/decisions-2026.md#shift-return-writes-a-line-on-the-mac).
...
- **The Settings window.** Each pane is the system's grouped form, as tall as what it holds up to
  620 points — [Decisions](archive/decisions-2026.md#on-the-mac-a-settings-page-is-the-systems-grouped-form).
